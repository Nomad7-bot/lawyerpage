...
export default function ReservationCheckPage() {
  ...
  return (
    <>
      ...
      {/* ── 취소 확인 모달 ── */}
      {showCancelModal && (
        <ReservationOverlay onClose={() => !cancelMutation.isPending && setShowCancelModal(false)}>
      ...
      {/* ── 일정 변경 모달 ── */}
      {showChangeModal && (
        <ReservationOverlay onClose={() => !changeMutation.isPending && setShowChangeModal(false)}>
          <div className="sm:hidden flex justify-center pt-3 pb-1">
            <div className="w-10 h-1 bg-bg-light rounded-full" aria-hidden />
          </div>
